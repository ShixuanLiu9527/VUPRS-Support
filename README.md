...
| `A47`-`B47` | `AGND` | --- | 模拟信号地 |
| `A48`-`B48` | `DGND` | --- | 数字信号地 |
| `A49`-`B49` | `Card PRSNT#` | `I` | 麦克风整列插入检测引脚, 低电平有效 |
 
`A49`-`B49` 是麦克风阵列插入检测引脚, 当麦克风整列信号插入 `PCIe` 接口时, 该引脚会被拉低, 表示 "已经插入".  
为了实现插入检测功能, 麦克风阵列板端会连接 `A48`-`B48` 和 `A49`-`B49`, 在任何情况下都不要将 `A48`-`B48` 的数字地信号用于模拟系统.  
